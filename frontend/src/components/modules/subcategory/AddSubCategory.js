...
getCategories()
    ...
    const handleSubmit = (e) => {
        setIsLoading(true)
        axios.post(`${Constants.BASE_URL}/sub_category`, input).then(res => {
            setIsLoading(false)
            Swal.fire({
                position: 'top-end',
                icon: res.data.cls,
    ...
    return (
        <div>
            <BreadCrumb title={`New Sub Category`} />

            <div className="row">
                <div className='col-md-8'>
                    <div className='card'>
                        <div className='card-header'>
                           <CardHeader title={'Add Sub Category'} button_text={'List'} icon={<FaList/>} link={'/sub_category'}/>
                        </div>
