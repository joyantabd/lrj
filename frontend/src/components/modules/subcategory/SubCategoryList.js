...
const SubCategoryList = () => {
    ...
    const getSubCategories = (pageNumber) => {
        setIsLoading(true)
        axios.get(`${Constants.BASE_URL}/sub_category?page=${pageNumber}&order_by=${input.order_by}&search=${input.search}&per_page=${input.per_page}&dirrection=${input.direction}`).then(res => {
    ...
    const handleDelete = (id) =>{
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`${Constants.BASE_URL}/sub_category/${id}`).then(res=>{
                    Swal.fire({
                        position: 'top-end',
                        icon: res.data.cls,
                        title: res.data.msg,
                ...
                <div className='col-md-12'>
                    <div className='card mb-4'>
                        <div className='card-header'>
                            <CardHeader title={'Sub Category List'} button_text={'Add New'} icon={<BsPlusCircleFill />} link={'/sub_category/create'} />
                        </div>
                        <div className='card-body'>
                            <div className='search-area mb-4'>
                                <div className='row'>
                                    ...
                                    <button onClick={()=>getSubCategories(1)} className='btn btn-sm btn-primary'><FaSearch/> Search </button>
                                    </div>
                                    </div>
                                </div>
                            ...
                            <CategoryPhoto show={modalPhotoShow} size='' photo={modalPhoto} title='Sub Category Photo' onHide={() => setModalPhotoShow(false)} />
                            <CategoryView show={modalShow} size='' category={category}  title='SubCategory Details' onHide={() => setModalShow(false)} />
                        </div>
                            }
