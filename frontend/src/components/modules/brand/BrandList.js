...
const BrandList = () => {
    ...
    const getBrands = (pageNumber) => {
        setIsLoading(true)
        axios.get(`${Constants.BASE_URL}/brands?page=${pageNumber}&order_by=${input.order_by}&search=${input.search}&per_page=${input.per_page}&dirrection=${input.direction}`).then(res => {
                ...
                <div className='col-md-12'>
                    <div className='card mb-4'>
                        <div className='card-header'>
                            <CardHeader title={'Brand List'} button_text={'Add New'} icon={<BsPlusCircleFill />} link={'/brand/create'} />
                        </div>
                        <div className='card-body'>
                            <div className='search-area mb-4'>
                                <div className='row'>
                                    ...
                                    <button onClick={()=>getBrands(1)} className='btn btn-sm btn-primary'><FaSearch/> Search </button>
                                    </div>
                                    </div>
                                </div>
                                ...
                                <tbody>
                                    {Object.keys(datas).length>0 ? datas.map((data, index) => (
                                        <tr key={index}>
                                            ...
                                            <td>
                                                <p className='text-warning'>Name: {data.name}</p><p className='text-info'>Slug: {data.slug}</p>
                                            </td>
                                            <td>
                                                <p>POS-{data.serial}</p> <p className='text-danger'>{data.status}</p>
                                            </td>
                            ...
                            <CategoryPhoto show={modalPhotoShow} size='' photo={modalPhoto} title='Brand Photo' onHide={() => setModalPhotoShow(false)} />
                            <CategoryView show={modalShow} size='' category={brands}  title='Brand Details' onHide={() => setModalShow(false)} />
                        </div>
                            }
